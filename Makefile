...
SRC := src/main.c src/album.c src/inventory.c src/space.c src/space_physics.c src/space_units.c src/stellar.c src/world_environment.c src/ship.c src/nether.c src/entity.c src/ecology.c src/terrain.c src/planet_surface.c src/planet_material.c src/planet_renderer.c src/discovery.c src/chunks.c src/world.c src/player.c src/interaction.c src/render.c src/particles.c src/audio.c src/weather.c src/starmap.c
...
test: $(TEST_TARGET) $(PLANET_SURFACE_TEST_TARGET) $(PLANET_MATERIAL_TEST_TARGET) $(SPACE_PHYSICS_TEST_TARGET) $(SPACE_UNITS_TEST_TARGET) $(STELLAR_TEST_TARGET)
	./$(TEST_TARGET)
...
clean:
	rm -f $(TARGET) $(TEST_TARGET) $(PLANET_SURFACE_TEST_TARGET) $(PLANET_MATERIAL_TEST_TARGET) $(SPACE_PHYSICS_TEST_TARGET) $(SPACE_UNITS_TEST_TARGET) $(STELLAR_TEST_TARGET)
